...
export class QrService {
	...
	constructor(@Inject('logger') logger: Logger, @Inject('axios') axiosInstance: AxiosInstance) {
		this.#_logger = logger
	...
	async factorize(matrix: number[][]): Promise<{ Q: number[][]; R: number[][]; stats: any }> {
		...
		try {
			...
			const response = await this.#_axiosInstance.post(`${STATS_API_URL}`, {
				Q,
				R
			})

			this.#_logger.info({ message: `response ${JSON.stringify(response.data)}` })
